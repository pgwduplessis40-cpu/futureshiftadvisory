...
final class NewMessageNotification extends ChannelAwareNotification
{
    ...
    {
        $message = $this->message->loadMissing(['sender', 'thread.client']);
        $thread = $message->thread;
        $client = $thread?->client;
        $mail = (new MailMessage)
            ->subject('New message: '.$thread?->subject)
            ->line('A new message is available in Future Shift Advisory.')
            ->line('Client: '.($client?->legal_name ?? 'Client conversation'))
    ...
    {
        $message = $this->message->loadMissing('thread.client');
        $thread = $message->thread;
        $client = $thread?->client;

        if ($thread === null || $client === null) {
            return null;
        }
