...
final class SurveyLibrary
{
    ...
    {
        $survey = $this->defaultSurveyQuery()->first();
        ...
        try {
            return DB::transaction(function () use ($creator): Survey {
                $survey = $this->defaultSurveyQuery()->lockForUpdate()->first();
    ...
    {
        $survey = $this->serviceImprovementQuery()->first();
        ...
        try {
            return DB::transaction(function () use ($creator): Survey {
                $survey = $this->serviceImprovementQuery()->lockForUpdate()->first();
                ...
                foreach ($this->serviceImprovementQuestions() as $question) {
     ...
     */
    private function defaultSurveyQuery(): Builder
    {
        return Survey::query()
     ...
     * @return Builder<Survey>
     */
    private function serviceImprovementQuery(): Builder
    {
        return Survey::query()
            ->where('key', self::SERVICE_IMPROVEMENT_KEY)
     ...
     * @return array<int, array<string, mixed>>
     */
    private function serviceImprovementQuestions(): array
    {
        return [
            [
                'order' => 1,
