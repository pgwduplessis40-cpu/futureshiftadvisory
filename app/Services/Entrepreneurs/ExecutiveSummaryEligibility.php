...
final class ExecutiveSummaryEligibility
{
    ...
    public function __construct(private readonly ExecutiveSummaryContext $contexts) {}
    ...
    public function evaluate(BusinessPlan $plan, EntrepreneurProfile $profile): array
    {
        ...
        return ['eligible' => true, 'reason' => null, 'assessment_id' => (string) $assessment->getKey(), 'context_hash' => $contextHash, 'assessed_context_hash' => $assessedContextHash];
    }
    ...
    public function require(BusinessPlan $plan, EntrepreneurProfile $profile): PlanAssessment
    {
        ...
        if (! $eligibility['eligible'] || $eligibility['assessment_id'] === null) {
            throw ValidationException::withMessages(['executive_summary' => $eligibility['reason'] ?? 'The executive summary is not eligible for generation.']);
        }
    ...
    {
        return ['eligible' => false, 'reason' => $reason, 'assessment_id' => $assessment?->getKey() === null ? null : (string) $assessment->getKey(), 'context_hash' => $contextHash, 'assessed_context_hash' => $assessedContextHash];
    }
}
