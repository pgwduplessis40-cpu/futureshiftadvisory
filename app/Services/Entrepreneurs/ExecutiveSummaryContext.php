...
/** Builds the plan-and-budget fingerprint assessed before a summary is issued. */
final class ExecutiveSummaryContext
{
    public function hash(BusinessPlan $plan, EntrepreneurProfile $profile): string
    {
        ...
        return hash('sha256', json_encode($this->normalise([
            'profile' => [
                'name' => $profile->name,
                'company_name' => $profile->company_name,
                'concept_summary' => $profile->concept_summary,
            ],
            'sections' => $this->sourceSections($plan)->map(fn (PlanSection $section): array => [
                'key' => $section->key,
                'title' => $section->title,
                'body' => $section->body,
    ...
    private function sourceSections(BusinessPlan $plan): Collection
    {
        return $plan->sections
            ->reject(fn (PlanSection $section): bool => (string) data_get($section->metadata, 'requirement_key') === BusinessPlanExecutiveSummary::REQUIREMENT_KEY
        ...
        foreach ($value as $key => $item) {
            if (is_array($item)) {
                $value[$key] = array_is_list($item)
                    ? array_map(fn (mixed $child): mixed => is_array($child) ? $this->normalise($child) : $child, $item)
                    : $this->normalise($item);
            }
        }
