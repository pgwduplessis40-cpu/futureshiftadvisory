...
final class GenerateEligibleExecutiveSummary implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(RequestContext $context, ExecutiveSummaryEligibility $eligibility, BusinessPlanExecutiveSummary $summaries): void
    {
        ...
        try {
            $assessment = PlanAssessment::query()->with('businessPlan.entrepreneurProfile')->find($this->assessmentId);
            ...
            if ((bool) ($summaries->status($plan, $profile)['usable'] ?? false)) {
                return;
            }
