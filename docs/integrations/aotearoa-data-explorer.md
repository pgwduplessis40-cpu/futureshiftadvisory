...
This integration uses Stats NZ Aotearoa Data Explorer (ADE) for industry and business benchmark tables only. Macro indicators that ADE did not cover in the implementation test, such as CPI, GDP, and unemployment, are governed through manual reference data and projected into `economic_indicators` only after the linked learning update is implemented.
...
`config('integrations.stats_nz.datasets')` is the source of truth for ADE dataflows:
...
The ADE client loops the configured datasets, calls `ResilientHttp::request()` with explicit headers, parses SDMX-JSON v1.0 observation data, and returns records through `StatsNzClient::industryBenchmarks()`.
