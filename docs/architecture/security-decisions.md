...
| SD-03 | Upload malware scanning fails closed. Production must use `ClamAvScanner` (`FEATURE_VIRUS_SCAN_LIVE=true`); `NoopScanner` is ignored outside local/testing even if configured. If live scanning is unavailable, uploads are quarantined and hidden from clients. | Active design. | Before production cutover, verify ClamAV daemon health and env values. |
