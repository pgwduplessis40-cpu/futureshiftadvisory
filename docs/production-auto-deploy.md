...
4. `deploy.sh` checks out that exact commit, builds the client and SSR bundles, runs migrations, installs and verifies the Laravel scheduler timer, runs the authenticated operational-health checks, restarts PHP-FPM and SSR, and verifies SSR.
...
The deploy user needs write access to the application checkout and permission to manage the `futureshift-scheduler.timer` unit and restart the PHP-FPM and SSR services through `sudo systemctl`. Do not use the VPS root password or place it in GitHub.
