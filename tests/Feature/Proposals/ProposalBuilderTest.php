...
final class ProposalBuilderTest extends TestCase
{
    ...
    public function test_proposal_generation_uses_active_uploaded_proposal_template(): void
    {
        ...
        $this->assertStringContainsString('proposal-generated-content', $this->renderer->html);
        $this->assertStringContainsString('padding: 0 25.4mm 18mm;', $this->renderer->html);
        $this->assertStringContainsString('padding-top: 18mm;', $this->renderer->html);
        $this->assertStringContainsString('<h2>Scope</h2>', $this->renderer->html);
        $coverPosition = strpos($this->renderer->html, 'UPLOADED PROPOSAL TEMPLATE');
        $generatedPagePosition = strpos($this->renderer->html, '<div class="uploaded-docx-report-template proposal-generated-page">');
        $scopePosition = strpos($this->renderer->html, '<h2>Scope</h2>');
        $this->assertNotFalse($coverPosition);
        $this->assertNotFalse($generatedPagePosition);
        $this->assertNotFalse($scopePosition);
        $this->assertLessThan($generatedPagePosition, $coverPosition);
        $this->assertLessThan($scopePosition, $generatedPagePosition);
