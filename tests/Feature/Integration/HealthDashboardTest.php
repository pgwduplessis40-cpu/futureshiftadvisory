    ...
    public function test_super_admin_can_view_latest_integration_health_samples(): void
    {
        ...
        $this->actingAsMfa($admin)
            ...
            ->assertInertia(fn (Assert $page): Assert => $page
                ...
                ->where('governanceNotices.0.service', 'IRD')
                ->where('governanceNotices.0.label', 'Deferred pending IRD Data Consumer category')
                ->where('governanceNotices.0.note', fn (string $note): bool => str_contains($note, 'Data Consumer'))
                ...
                ->where('aiUsage.previous_month.requests', 1)
                ->where('aiUsage.previous_month.total_tokens', 5_500)
                ->where('aiUsage.previous_month.estimated_cost_usd', 0.0225)
                ->where('aiUsage.periods.month_label', 'June 2026')
                ->where('aiUsage.periods.previous_month_label', 'May 2026')
