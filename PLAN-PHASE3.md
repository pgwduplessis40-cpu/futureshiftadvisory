...
**Carryover owner inputs that now hard-block Phase 3 surfaces:** Anthropic API key (DD/entrepreneur AI), Stripe + Windcave merchant accounts (commerce — onboarding takes weeks, start now), FSP Register access (broker validation), PPSR/LINZ/IPONZ access (DD legal workstream), `Business_Plan_Rating_Matrix.pdf` (entrepreneur founding criteria — required by WO-87), lawyer-reviewed T&C + panel agreement + DD disclaimer text, Whisper credentials (voice-to-text). Tracked in §12.
...
### 7.3 Entrepreneur rating framework (WO-87) — admin-managed, learning-driven
...
#### WO-87 — Admin-managed rating framework (founding PDF)
**Spec refs:** §17.4; §17.6; Appendix C
**Goal:** Seed the 11 founding criteria from `docs/rating-criteria/Business_Plan_Rating_Matrix.pdf`; admin-managed criteria/weights/descriptors/industry-variants, versioned; evolves only via the governed learning queue (3 dimensions).
**Depends on:** Phase 2 learning queue scaffolding; owner-supplied PDF.
**Key files:** `rating_frameworks`/`rating_criteria` tables/models, seeder from the PDF, admin framework editor.
**Acceptance:** 11 criteria seeded; admin can edit weights/descriptors/variants without code; changes versioned; learning-driven changes go through the governed queue (no hardcoding, no silent update).
**Tests:** seed from PDF; admin edit + version; governed-change path.
**Out of scope:** scoring (WO-88).
...
**Depends on:** WO-87, WO-86, Phase 2 WO-31 (spine), WO-32 (feedback).
**Key files:** `plan_assessments` table/model, `app/Services/Entrepreneurs/Assessment.php`.
**Acceptance:** AI scores all 11; advisor adjustment requires a note and emits a governed learning candidate; private advisory note never visible to the entrepreneur; criteria hidden until finalisation.
...
| P3-R3 | `Business_Plan_Rating_Matrix.pdf` required to seed the founding 11 criteria | Owner supply to `docs/rating-criteria/` | Before WO-87 |
...
| Admin-managed rating framework | WO-87 |
