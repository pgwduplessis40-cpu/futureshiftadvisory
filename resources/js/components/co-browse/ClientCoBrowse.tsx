    ...
    useEffect(() => {
        ...
        const poll = (): void => {
            ...
            void coBrowsePost<{ actions: GuidanceAction[] }>(
                replaceCoBrowsePath(
                    config.pending_actions_url,
                    '__session__',
                    session.id,
                ),
                {
                    ...coBrowseParticipant(credentials),
                    after_id: lastActionId.current,
                },
            )
                .then(({ actions }) => {
                    if (mounted) {
        ...
        <>
                ...
                >
                    {error ? (
                        <>
                            <DialogHeader>
                                <DialogTitle>
                                    Guided assistance is unavailable
                                </DialogTitle>
                        ...
                        <>
                            <DialogHeader>
                                <DialogTitle>
                                    Allow guided assistance?
                                </DialogTitle>
                                <DialogDescription>
                                    {prompt?.advisor_name} can point and
                                    highlight items only inside{' '}
                                    {prompt?.context.label}. They cannot control
                                    your mouse or keyboard, open other sites, or
                                    see your screen.
                                </DialogDescription>
                            </DialogHeader>
                            <DialogFooter>
            ...
            {session?.status === 'active' ? (
                <div className="fixed inset-x-0 top-0 z-50 flex items-center justify-between gap-3 bg-amber-400 px-4 py-2 text-sm font-medium text-black shadow">
                    <span>
                        Your advisor can point and highlight items on this
                        Future Shift Advisory page. You can stop this at any
                        time.
                    </span>
                    <Button
                        size="sm"
    ...
    useEffect(() => {
        const update = (): void => {
            ...
            const target = document.querySelector<HTMLElement>(
                `[data-co-browse-target="${highlight}"]`,
            );
        ...
        >
            {pointer ? (
                <span
                    ...
                    style={{
                        left: `calc(${pointer.x * 100}% - 10px)`,
                        top: `calc(${pointer.y * 100}% - 10px)`,
                    }}
                />
            ) : null}
            {rect ? (
