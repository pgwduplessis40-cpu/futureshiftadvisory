...
export default function EntrepreneursShow({
    ...
    const fixedCostTrace =
        entrepreneur.latest_plan?.budget.fixed_cost_trace ?? [];
    const fixedCostCadenceRepairUrl =
        entrepreneur.latest_plan?.budget.fixed_cost_cadence_repair_url ??
        null;
    const [fixedCostRepairIndex, setFixedCostRepairIndex] = useState<
        number | null
    >(null);
    const [fixedCostRepairError, setFixedCostRepairError] = useState<
        string | null
    >(null);
    ...
    const repairDuplicateFixedCostCadence = (rowIndex: number) => {
        if (!fixedCostCadenceRepairUrl || fixedCostRepairIndex !== null) {
            return;
        }

        router.patch(
            fixedCostCadenceRepairUrl,
            { row_index: rowIndex },
            {
                preserveScroll: true,
                onStart: () => {
                    setFixedCostRepairIndex(rowIndex);
                    setFixedCostRepairError(null);
                },
                onError: (errors) => {
                    const error = errors.row_index;
                    setFixedCostRepairError(
                        typeof error === 'string'
                            ? error
                            : 'The stored fixed-cost row could not be corrected. Refresh the audit and try again.',
                    );
                },
                onFinish: () => setFixedCostRepairIndex(null),
            },
        );
    };
                                ...
                                <div className="flex flex-wrap items-start justify-between gap-3 border-b p-4">
                                    <div>
                                        ...
                                        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
                                            These are the live stored rate,
                                            number of parallel units, billing
                                            cadence, and monthly equivalent
                                            used by the funding calculation.
                                            A cadence-period count in Qty would
                                            convert the same period twice.
                                        </p>
                                    </div>
                                    <Badge
                                        variant={
                                    ...
                                    >
                                        {
                                            fixedCostTrace.filter(
                                                (row) =>
                                                    row.duplicate_cadence_quantity,
                                            ).length
                                        }{' '}
                                        duplicate cadence count
                                        {fixedCostTrace.filter(
                                            (row) =>
                                                row.duplicate_cadence_quantity,
                                        ).length === 1
                                                    ...
                                                    <td className="px-3 py-3">
                                                        ...
                                                        fixedCostCadenceRepairUrl ? (
                                                            <Button
                                                                ...
                                                                variant="outline"
                                                                disabled={
                                                                    fixedCostRepairIndex !==
                                                                    null
                                                                }
                                                                onClick={() =>
                                                                    repairDuplicateFixedCostCadence(
                                                                        row.index,
                                                                    )
                                                                }
                                                            >
