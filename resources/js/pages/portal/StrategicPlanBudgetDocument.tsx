...
export default function StrategicPlanBudgetDocument({
    ...
    const planLabel =
        budget.label === 'Operating Plan & Budget'
            ? 'Operating plan'
            : 'Business plan';
    const computed = budget.computed ?? {};
    const cashSeries = budget.analytics.predictive.monthly_forecast.map(
        (row) => ({
            month: row.month,
                ...
                <article className="presentation-paper bg-white px-6 py-8 text-slate-900 shadow-sm sm:px-10 sm:py-12">
                    <section className="presentation-section border-b-4 border-[#D4A91F] pb-9">
                        <div className="flex flex-wrap items-start justify-between gap-6">
                            ...
                            <div className="min-w-44 border-l-2 border-[#263D68] pl-4 text-sm text-slate-600">
                                <p className="font-medium text-[#1F3458]">
                                    Client document
                                </p>
                                <p className="mt-2">
                        ...
                        <p className="mt-8 max-w-4xl text-base leading-7 text-slate-700">
                            A single presentation of the{' '}
                            {planLabel.toLowerCase()}, financial budget, and
                            decision-ready insights for {businessName}.
                        </p>
                    ...
                    <section className="presentation-section pt-10">
                        ...
                        <div className="mt-8 divide-y divide-slate-200 border-y border-slate-200">
                            {budget.business_plan_sections.map((section) => (
                                <article
                                    key={section.key}
                                    className="grid gap-3 py-6 md:grid-cols-[13rem_minmax(0,1fr)]"
                                >
                                    <div>
                                        <h3 className="text-sm font-semibold text-[#1F3458]">
                                            {section.title}
                                        </h3>
                    ...
                    <section className="presentation-section presentation-break pt-12">
                        <SectionHeading
                            ...
                            number="02"
                            title="Budget"
                            subtitle="All values are NZD and GST exclusive. Forecast figures reflect the current saved assumptions."
                        />
                        <div className="mt-7 grid border-y border-slate-200 sm:grid-cols-2 lg:grid-cols-3">
                            <DocumentMetric
                                label="Implementation costs"
                                value={formatCurrency(
                                    computed.total_launch_costs,
                                )}
                            />
                            <DocumentMetric
                            ...
                            <DocumentMetric
                                ...
                                value={formatCurrency(
                                    computed.available_after_launch,
                                )}
                            />
                            <DocumentMetric
                                label="Break-even"
                            ...
                            <DocumentMetric
                                ...
                                value={formatYear(
                                    computed.cash_flow_positive_year,
                                )}
                            />
                        </div>

                        <div className="mt-9">
                            <h3 className="text-base font-semibold text-[#1F3458]">
                        ...
                        <div className="mt-10 space-y-10">
                            <BudgetTable
                                ...
                                rows={budget.implementation_costs}
                            />
                            <BudgetTable
                                title="Monthly operating costs"
                                rows={budget.monthly_fixed_costs}
                            />
                            <RevenueTable rows={budget.revenue_forecast} />
                            <BudgetTable
                                title="Funding sources"
                                rows={budget.funding_sources}
                            />
                            <FutureCostTable rows={budget.future_costs} />
                            <FundingScenarioTable
                                rows={budget.funding_scenarios}
                            />
                            <AnnualForecastTable
                                rows={
                                    budget.analytics.predictive.annual_forecast
                                }
                            />
                        </div>
                    </section>
                    ...
                    <section className="presentation-section presentation-break pt-12">
                        <SectionHeading
                            ...
                            number="03"
                            title="Insights"
                            subtitle="The current readout turns the plan and budget into decisions, risks, forecasts, and next actions."
                        />
                        ...
                        {cashSeries.length > 0 ? (
                            <div className="mt-10">
                                <BudgetCashChart
                                    ...
                                    breakEvenMonth={null}
                                    runwayMonths={
                                        computed.runway_months ?? null
                                    }
                                    runwayOpenEnded={
                                        computed.runway_open_ended ?? false
                                    }
                        ...
                        <div className="mt-10 grid gap-5 lg:grid-cols-2">
                            <MarginChart
                                rows={
                                    budget.analytics.charts.margin_percentages
                                }
                            />
                            <AnnualRevenueCostsChart
                                rows={
                                    budget.analytics.charts.annual_revenue_costs
                                }
                            />
                            <ScenarioImpactChart
                                rows={
                                    budget.analytics.charts.scenario_comparison
                                }
                            />
                            <ConfidenceMixChart
                                rows={budget.analytics.charts.confidence_mix}
                            />
                        </div>

                        <div className="mt-10 grid gap-8 lg:grid-cols-2">
                            <ScenarioTable
                                rows={budget.analytics.predictive.scenarios}
                            />
                            <ActionList
                                actions={budget.analytics.prescriptive.actions}
                            />
                        </div>
                        ...
                        <div className="mt-10 border-t border-slate-200 pt-7">
                            ...
                            <ul className="mt-4 grid gap-3 text-sm leading-6 text-slate-700 md:grid-cols-2">
                                {budget.analytics.prescriptive.advisor_decision_points.map(
                                    (point) => (
                                        <li
                                            key={point}
                                            className="border-l-2 border-[#D4A91F] pl-3"
                                        >
                                            {point}
                                        </li>
                                    ),
                    ...
                    <footer className="mt-14 border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">
                        Prepared for {businessName} by Future Shift Advisory.
                        This planning document reflects the saved plan and
                        budget at the preparation time shown above and should be
                        reviewed when assumptions, funding, or financial
                        evidence change.
                    </footer>
                </article>
            </main>
...
function SectionHeading({
    ...
    return (
        <div className="flex gap-4">
            <div className="flex size-9 shrink-0 items-center justify-center bg-[#263D68] text-xs font-semibold text-white">
                {number}
            </div>
            <div>
                <div className="flex items-center gap-2">
                    <Icon
                        className="size-4 text-[#D4A91F]"
                        aria-hidden="true"
                    />
                    <h2 className="text-2xl font-semibold text-[#1F3458]">
                        {title}
                    </h2>
                </div>
...
function DocumentMetric({ label, value }: { label: string; value: string }) {
    return (
        <div className="border-slate-200 px-4 py-4 first:pl-0 last:border-r-0 sm:border-r">
...
function BudgetTable({ title, rows }: { title: string; rows: BudgetRow[] }) {
    ...
    return (
        <TableSection
            title={title}
            emptyLabel={`No ${title.toLowerCase()} have been recorded.`}
            hasRows={populated.length > 0}
        >
                ...
                <tbody>
                    {populated.map((row, index) => (
                        ...
                        >
                            ...
                            <td className="py-2.5 text-right text-slate-600 capitalize">
...
function RevenueTable({ rows }: { rows: BudgetRow[] }) {
    ...
    return (
        <TableSection
            ...
            emptyLabel="No revenue forecast lines have been recorded."
            hasRows={populated.length > 0}
        >
            <table className="presentation-table w-full border-collapse text-left text-sm">
                <thead>
                ...
                <tbody>
                    {populated.map((row, index) => (
                        ...
                        >
                            <td className="py-2.5 pr-3 text-slate-800">
                                {row.label || 'Untitled revenue line'}
                            </td>
                            <td className="py-2.5 pr-3 text-right tabular-nums">
                                {formatCurrency(row.amount)}
                            </td>
                            <td className="py-2.5 pr-3 text-right">
                                Month {formatNumber(row.month ?? 1)}
                            </td>
                            <td className="py-2.5 pr-3 text-right tabular-nums">
                                {formatPercent(row.monthly_growth_percent)}
                            </td>
                            <td className="py-2.5 text-right tabular-nums">
                                {formatPercent(row.gross_profit_percent)}
                            </td>
                        </tr>
                    ))}
                </tbody>
...
function FutureCostTable({ rows }: { rows: BudgetRow[] }) {
    ...
    return (
        <TableSection
            title="Future costs"
            emptyLabel="No future costs have been recorded."
            hasRows={populated.length > 0}
        >
            <table className="presentation-table w-full border-collapse text-left text-sm">
                <thead>
...
function FundingScenarioTable({ rows }: { rows: FundingScenario[] }) {
    const populated = rows.filter(
        (row) => Boolean(row.name?.trim()) || toNumber(row.amount) !== 0,
    );

    return (
        <TableSection
            title="Funding scenarios"
            emptyLabel="No funding scenarios have been recorded."
            hasRows={populated.length > 0}
        >
            <table className="presentation-table w-full border-collapse text-left text-sm">
                <thead>
                ...
                <tbody>
                    {populated.map((row, index) => (
                        <tr
                            key={`funding-scenario-${index}`}
                            className="border-b border-slate-200 last:border-b-0"
                        >
                            <td className="py-2.5 pr-3 text-slate-800">
                                {row.name || 'Untitled scenario'}
                                {row.type ? ` (${formatLabel(row.type)})` : ''}
                            </td>
                            <td className="py-2.5 pr-3 text-right tabular-nums">
                                {formatCurrency(row.amount)}
                            </td>
                            <td className="py-2.5 pr-3 text-right">
                                Year {formatNumber(row.year ?? 1)}
                            </td>
                            <td className="py-2.5 text-right text-slate-600">
                                {fundingTerms(row)}
                            </td>
...
function AnnualForecastTable({ rows }: { rows: AnnualForecast[] }) {
    return (
        <TableSection
            ...
            emptyLabel="No forecast has been generated yet."
...
function ScenarioTable({ rows }: { rows: Scenario[] }) {
    return (
        <section className="presentation-section">
            ...
            {rows.length === 0 ? (
                <p className="mt-3 text-sm text-slate-500">
                    No scenario comparisons are available yet.
                </p>
            ) : (
                <div className="mt-3 border-y border-slate-200 py-1">
                    <table className="presentation-table w-full border-collapse text-left text-sm">
                ...
                <ol className="mt-3 space-y-4">
                    {actions.map((action, index) => (
                        <li
                            ...
                            className="grid grid-cols-[1.75rem_minmax(0,1fr)] gap-3 text-sm"
                        >
                            <span className="flex size-7 items-center justify-center bg-[#263D68] text-xs font-semibold text-white">
                                {index + 1}
                            </span>
                            <div>
                                <p className="font-medium text-slate-800">
        ...
        >
            {rows.length === 0 ? (
                ...
                rows.map((row) => (
                    <div
                        key={row.label}
                        className="mt-4 grid grid-cols-[3.5rem_minmax(0,1fr)] gap-3"
                    >
                        <span className="pt-0.5 text-xs font-semibold text-slate-700">
                            {row.label}
                        </span>
...
function AnnualRevenueCostsChart({
    rows,
}: {
    rows: InsightCharts['annual_revenue_costs'];
}) {
        ...
        >
            {rows.length === 0 ? (
                ...
                rows.map((row) => (
                    <div
                        key={row.label}
                        className="mt-4 grid grid-cols-[3.5rem_minmax(0,1fr)_4.5rem] items-center gap-3"
                    >
                        ...
                        <span
                            className={`text-right text-xs font-semibold ${row.net_cash_flow < 0 ? 'text-red-600' : 'text-emerald-700'}`}
                        >
...
function ScenarioImpactChart({
    ...
    rows: InsightCharts['scenario_comparison'];
}) {
...
function ConfidenceMixChart({
    ...
    rows: InsightCharts['confidence_mix'];
}) {
                ...
                <>
                    <div className="mt-5 flex h-3 overflow-hidden rounded-sm bg-slate-100">
                        {rows.map((row) => (
                            <span
                                key={row.label}
                                className={
                                    toneByLabel[row.label] ?? 'bg-slate-300'
                                }
                                style={{
                    ...
                    <div className="mt-5 space-y-3">
                        {rows.map((row) => (
                            <div
                                key={row.label}
                                className="grid grid-cols-[minmax(0,1fr)_3rem_3.5rem] items-center gap-3 text-xs"
                            >
                                <span className="flex items-center gap-2 font-medium text-slate-700">
                                    <span
                                        className={`size-2 ${toneByLabel[row.label] ?? 'bg-slate-300'}`}
                                    />
                                    {row.label}
                                </span>
    ...
    return (
        <div className="grid grid-cols-[3.25rem_minmax(0,1fr)_3.75rem] items-center gap-2 text-[11px]">
            ...
            <span className="block h-1.5 overflow-hidden bg-slate-100">
                <span
                    className={`block h-full ${value < 0 ? 'bg-red-500' : tone}`}
                    style={{ width: `${width}%` }}
                />
            </span>
...
function populatedRows(rows: BudgetRow[]): BudgetRow[] {
    return rows.filter(
        (row) => Boolean(row.label?.trim()) || toNumber(row.amount) !== 0,
    );
}

function fundingTerms(row: FundingScenario): string {
    const terms = [
        row.interest_rate_percent === undefined
            ? null
            : `${formatNumber(row.interest_rate_percent)}% interest`,
        row.term_years === undefined
            ? null
            : `${formatNumber(row.term_years)} years`,
        row.investor_equity_percent === undefined
            ? null
            : `${formatNumber(row.investor_equity_percent)}% equity`,
...
function formatNumber(value: number | string | null | undefined): string {
    return new Intl.NumberFormat('en-NZ', { maximumFractionDigits: 1 }).format(
        toNumber(value),
    );
}
