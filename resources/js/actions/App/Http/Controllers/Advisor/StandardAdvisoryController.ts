 ...
 */
export const confirmWebsiteUrl = (
    args:
        | { client: string | { id: string } }
        | [client: string | { id: string }]
        | string
...
confirmWebsiteUrl.url = (
    ...
    return (
        confirmWebsiteUrl.definition.url
            .replace('{client}', parsedArgs.client.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
...
const StandardAdvisoryController = {
    runAnalysis,
    confirmWebsiteUrl,
    generatePack,
};
