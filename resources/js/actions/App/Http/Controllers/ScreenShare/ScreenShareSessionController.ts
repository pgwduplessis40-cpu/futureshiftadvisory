...
browserPermission.url = (
    ...
    return (
        browserPermission.definition.url
            .replace('{session}', parsedArgs.session.toString())
            .replace(/\/+$/, '') + queryParams(options)
    );
};
...
const ScreenShareSessionController = {
    store,
    respond,
    browserPermission,
    active,
